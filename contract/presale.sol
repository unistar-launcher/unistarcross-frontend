...
contract HeconPresale is Ownable {
  ...
  uint256 public constant INVESTMENT_LIMIT_PRESALE = 120  ether; // 120 HT is maximum investment limit for pre-sale
  uint256 public constant INVESTMENT_LIMIT_DEVELOPER = 102 ether; // 102 HT is maximum investment limit for developer pre-sale
  uint256 public constant INVESTMENT_LIMIT_PUBLIC = 100 ether;    // 100 HT is maximum investment limit for public pre-sale

  uint256 public constant INVESTMENT_RATIO_PRESALE   = 90; // whitelist pre-sale rate is 1 HT = 90 ZERO
  uint256 public constant INVESTMENT_RATIO_DEVELOPER = 140; // developer pre-sale rate is 1 HT = 140 ZERO
  uint256 public constant INVESTMENT_RATIO_PUBLIC = 60;   // public sale rate is 1 HT = 60 ZERO (Juncture listing price)
  ...
  modifier eligibleForPresale() {
    require(whitelistAddresses[_msgSender()] || devAddresses[_msgSender()] || allowPublicInvestment, "Your address is not whitelisted for either presale, or the public sale not started yet.");
    _;
  }
  ...
  function presale(uint256 _amount)
    ...
    eligibleForPresale
  {
    uint256 addressTotalInvestment = investments[_msgSender()].add(_amount);

    uint256 amountOfTokens;

    if (isDevAddress(_msgSender()) && !allowPublicInvestment){

      require(addressTotalInvestment <= INVESTMENT_LIMIT_DEVELOPER, "Max investment per dev pre-sale address is 102 HT.");

      amountOfTokens = _amount.mul(INVESTMENT_RATIO_DEVELOPER);

    } else if (isWhitelisted(_msgSender()) && !allowPublicInvestment) {

      require(addressTotalInvestment <= INVESTMENT_LIMIT_PRESALE, "Max investment per whitelist pre-sale address is 120 HT.");

      amountOfTokens = _amount.mul(INVESTMENT_RATIO_PRESALE);

    } else if (allowPublicInvestment) {

      require(addressTotalInvestment <= INVESTMENT_LIMIT_PUBLIC, "Max investment for every address (once public sale has started) is 100 HT.");

      amountOfTokens = _amount.mul(INVESTMENT_RATIO_PUBLIC);
    }
  ...
  function getPresaleRate(address userAddress) public view returns (uint256){
	if (isDevAddress(userAddress)) return 140;
		else if (isWhitelisted(userAddress)) return 90;
			else return 60;
	}
}